...
void initialize() {
  ...
  Kerry.addAutons( //Adds the autos under one term. Any order works
    autons("L Simple", exampleBlue, exampleRed, true), //The example for regular autons (true is only used once (sets default selected))
    ...
    utilAutons("Turn PID Cal", calibratePID), //Calibrates the turn PID (needs SD card)
...
void opcontrol() {
  ...
  if (Kerry.skills_is_selected()) chassis.odom_xyt_set(0_in, 0_in, 0_deg); //Sets position if skills (for driver control)
  ...
  while (true) {
    ...
    if (master.get_digital_new_press(DIGITAL_B)) { //On a new click of B
      driveSwitch(); //Switches the drive style
    }
